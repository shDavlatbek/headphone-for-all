...
//!
//! The process id of a playback stream is resolved from the node's
//! `application.process.id` (set by `pipewire-pulse` for PulseAudio clients), then the owning
//! client's `application.process.id`, then the client's kernel-verified `pipewire.sec.pid`.
//!
...
streams (Stream/Output/Audio) directly into the capture stream, so they exclude this process but \
also miss sounds that are not application streams (e.g. loopback/filter-chain sources). \
Per-app capture includes child processes. Requires a running PipeWire daemon (pipewire-pulse \
setups included) and libpipewire-0.3 at run time.";
...
impl NodeFilter {
    fn matches(self, pid: u32, parent_of: &dyn Fn(u32) -> Option<u32>) -> bool {
        match self {
            NodeFilter::AllExcept(excluded) => pid != excluded,
            NodeFilter::ProcessTree(root) => pid == root || is_descendant(pid, root, parent_of),
        }
    }
}
...
struct ClientModel {
    /// `pipewire.sec.pid` (kernel-verified peer pid; for `pipewire-pulse` clients this is the
    /// pulse server itself).
    sec_pid: Option<u32>,
    /// `application.process.id` from the client info.
    app_pid: Option<u32>,
    /// `application.name`.
...
impl GraphModel {
    /// Process id of a playback stream node, once its info has arrived.
    fn node_pid(&self, node_id: u32) -> Option<u32> {
        ...
        node.app_pid
            .or_else(|| client.and_then(|c| c.app_pid))
            .or_else(|| client.and_then(|c| c.sec_pid))
    }
    ...
    /// Processes with playback streams, deduplicated by pid, `own_pid` excluded, sorted by
    /// name (case-insensitive) then pid.
    fn apps(&self, own_pid: u32) -> Vec<CaptureApp> {
        let mut by_pid: BTreeMap<u32, String> = BTreeMap::new();
        for &node_id in self.nodes.keys() {
            let Some(pid) = self.node_pid(node_id) else {
                continue;
            };
            if pid == own_pid {
                continue;
    ...
    fn plan_links(
        ...
        for &node_id in self.nodes.keys() {
            ...
            if !filter.matches(pid, parent_of) {
                continue;
            }
            let outputs = self
                .ports
            ...
            .error({
                ...
                move |id, _seq, res, message| {
                    if id == pw::core::PW_ID_CORE {
                        tracing::warn!(res, message, "PipeWire core error");
...
struct LinkCapture {
    ...
    own_node: Option<u32>,
    links: HashMap<PlannedLink, pw::link::Link>,
}
...
impl Tracker {
    ...
    fn on_global(
        ...
        match global.type_ {
            ObjectType::Client => {
                tracker.model.clients.insert(
                    id,
                    ClientModel {
                        sec_pid: props.get("pipewire.sec.pid").and_then(parse_pid),
                        app_pid: None,
                ...
                match tracker.registry.bind::<pw::client::Client, _>(global) {
                            ...
                            .info(move |info| {
                                ...
                                with_tracker(&weak, |t| {
                                    if let Some(client) = t.model.clients.get_mut(&id) {
                                        if let Some(pid) =
                                            props.get("application.process.id").and_then(parse_pid)
                                        {
                                            client.app_pid = Some(pid);
                                        }
                                        if let Some(name) = props.get("application.name") {
                                            client.app_name = Some(name.to_owned());
                                        }
                                    }
                                    t.reconcile();
                                });
                    ...
                    Err(e) => tracing::debug!(id, error = %e, "cannot bind PipeWire client"),
                }
            }
            ObjectType::Node if props.get("media.class") == Some(OUTPUT_STREAM_CLASS) => {
                tracker.model.nodes.insert(
                    id,
                    NodeModel {
                        client_id: props.get("client.id").and_then(|v| v.trim().parse().ok()),
                        app_pid: None,
                        app_name: props.get("application.name").map(str::to_owned),
                                    ...
                                    if let Some(node) = t.model.nodes.get_mut(&id) {
                                        if let Some(props) = props {
                                            if let Some(pid) = props
                                                ...
                                                .and_then(parse_pid)
                                            {
                                                node.app_pid = Some(pid);
                                            }
                                            if let Some(name) = props.get("application.name") {
                                                node.app_name = Some(name.to_owned());
                                            }
                                        }
                                        node.info_seen = true;
                                    }
    ...
    fn on_global_remove(&mut self, id: u32) {
        ...
        if let Some(capture) = self.capture.as_mut() {
            // The server already destroyed links touching a removed node/port.
            capture
                .links
                .retain(|link, _| link.out_node != id && link.out_port != id && link.in_port != id);
        }
        self.reconcile();
    }
    ...
    /// Creates missing links and destroys unwanted ones (linked capture only).
    fn reconcile(&mut self) {
        ...
        let planned = model.plan_links(own_node, capture.filter, &read_ppid);
        let stale: Vec<PlannedLink> = capture
            .links
            .keys()
            .filter(|link| !planned.contains(link))
            .copied()
            .collect();
        for key in stale {
            if let Some(link) = capture.links.remove(&key) {
                if let Err(e) = capture.core.destroy_object(link) {
                    tracing::debug!(?key, error = %e, "cannot destroy PipeWire link");
                }
            }
        }
        for key in planned {
            if capture.links.contains_key(&key) {
                continue;
            }
                ...
                .create_object::<pw::link::Link>(link_factory, &props)
            {
                Ok(link) => {
                    ...
                    capture.links.insert(key, link);
                }
                Err(e) => tracing::warn!(?key, error = %e, "cannot create PipeWire link"),
            }
        }
    }
}
...
mod tests {
    ...
    fn filters_select_the_right_processes() {
        let parents = [(30, 20), (20, 10)];
        let parent_of = fake_parents(&parents);
        assert!(NodeFilter::AllExcept(5).matches(6, &parent_of));
        assert!(!NodeFilter::AllExcept(5).matches(5, &parent_of));
        assert!(NodeFilter::ProcessTree(10).matches(10, &parent_of));
        assert!(NodeFilter::ProcessTree(10).matches(30, &parent_of));
        assert!(!NodeFilter::ProcessTree(20).matches(10, &parent_of));
    }
    ...
    fn sample_graph(own_pid: u32) -> GraphModel {
        ...
        // Native clients: pid from the client info; the "pulse" client is pipewire-pulse.
        g.clients.insert(
            1,
            ClientModel {
                sec_pid: Some(100),
                app_pid: Some(100),
                app_name: Some("mpv".into()),
            },
        );
        g.clients.insert(
            2,
            ClientModel {
                sec_pid: Some(own_pid),
                app_pid: None,
                app_name: Some("hfa".into()),
            },
        );
        g.clients.insert(
            3,
            ClientModel {
                sec_pid: Some(900),
                app_pid: Some(900),
                app_name: Some("pipewire-pulse".into()),
            },
        );
        let node = |client: u32, app_pid: Option<u32>, name: Option<&str>| NodeModel {
        ...
        g.nodes.insert(10, node(1, None, Some("mpv"))); // pid 100 via client
        g.nodes.insert(11, node(2, None, None)); // our own playback
        g.nodes.insert(12, node(3, Some(300), Some("Firefox"))); // pulse: pid from node
        g.nodes.insert(13, node(1, None, Some("mpv"))); // second mpv stream (mono)
        g.nodes.insert(
            14,
            NodeModel {
                info_seen: false,
                ..node(1, None, Some("late"))
            },
        );
        // Ports.
        for (id, p) in [
    ...
    #[test]
    fn node_pids_resolve_node_then_client_info_then_sec_pid() {
        let own = 4242;
        let mut g = sample_graph(own);
        assert_eq!(g.node_pid(10), Some(100));
        assert_eq!(g.node_pid(11), Some(own), "falls back to pipewire.sec.pid");
        assert_eq!(
            g.node_pid(12),
            Some(300),
            "node application.process.id wins"
        );
        assert_eq!(g.node_pid(14), None, "no pid before the node info arrives");
        assert_eq!(g.node_pid(99), None);
        g.nodes.get_mut(&10).expect("node").app_pid = Some(101);
        assert_eq!(g.node_pid(10), Some(101));
    }
    ...
    fn exclude_self_links_every_other_stream() {
        ...
        let planned = g.plan_links(50, NodeFilter::AllExcept(own), &|_| None);
    ...
    fn process_capture_links_the_process_tree_only() {
        ...
        let parents = [(300, 100), (100, 1)];
        let parent_of = fake_parents(&parents);
        let planned = g.plan_links(50, NodeFilter::ProcessTree(100), &parent_of);
        let expected: BTreeSet<PlannedLink> = [
            link(10, 100, 500),
            link(10, 101, 501),
            link(12, 120, 500),
        ...
        let only_firefox = g.plan_links(50, NodeFilter::ProcessTree(300), &parent_of);
        assert_eq!(
            only_firefox,
        ...
        assert!(g
            .plan_links(77, NodeFilter::ProcessTree(300), &parent_of)
            .is_empty());
    }
    ...
    fn apps_are_deduplicated_sorted_and_exclude_self() {
        ...
        g.clients.insert(4, ClientModel::default());
        g.nodes.insert(
            15,
            NodeModel {
                client_id: Some(4),
    ...
    ///
    /// They need `pipewire` + `wireplumber` running for this user with a default sink, and
    /// `pw-play` on `PATH`. In a headless container:
    ///
    /// ```sh

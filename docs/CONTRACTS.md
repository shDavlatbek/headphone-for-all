...
`pipewire-live`) and `flutter.yml` (jobs `analyze`, `codegen`, `linux`, `android`, `windows`, `macos`, `ios`,
...
- `feat/cli`: `cargo run --release -p hfa-cli -- selftest --seconds 8 --loss 5 --jitter 20` must exit 0 on
...
- `feat/ffi`: `flutter_rust_bridge_codegen generate` (2.13.0, installed with cargo-binstall; cargo-expand
  alongside) run in `app/` after `flutter pub get` must leave `git status --porcelain` empty. The frb version is
...
- `feat/apple`: `flutter build ios --release --no-codesign` (builds `HfaBroadcast`; the runner has the Rust targets
  ...
  ios/Runner.xcworkspace -scheme Runner -destination id=<first available iPhone simulator>
...
- `feat/desktop`: packaging runs **only when the script exists** and with **no arguments**, after the release
  ...
  `packaging/dist/*.exe`), `bash packaging/linux/build-appimage.sh` (ubuntu-latest; `packaging/dist/*.AppImage`),
